...
processList = {
    f"REC_{particle}_{theta}deg_{momentum}GeV_1000evt_edm4hep": {
        "output": f"{particle}_{theta}deg_{momentum}GeV_1000evt"
    }
...
class RDFanalysis:
    def analysers(df):
        df2 = (
            df.Alias("MCTrackAssociations0", "_MCTruthSiTracksLink_sim.index")
            ...
            .Define(
                "trackStates_IP",
                "_SiTracks_Refitted_trackStates[_SiTracks_Refitted_trackStates.location == 1]",
            )
            .Define(
                "MC2TrackIndex",
                "MCTruthTrackIndex_full(MCTrackAssociations0, MCTrackAssociations1, MCTruthSiTracksLink.weight, MCParticles)",
            )
    ...
    def output():
        branchList = ["GunParticle", "GunParticleTSIP"]
        return branchList
